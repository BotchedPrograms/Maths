...
public class MixedRadix {
    ...
    public MixedRadix(int[] radices) {
        this.radices = new ArrayList<>();
        maxProduct = 1;
        for (int radix : radices) {
            this.radices.add(radix);
            maxProduct *= radix;
        }
    }
     ...
     */
    public MixedRadix(List<Integer> arrList) {
        this.radices = arrList;
        maxProduct = 1;
        for (int num : arrList) {
            maxProduct *= num;
        }
    ...
    public void addRadix(int num) {
        radices.add(num);
        maxProduct *= num;
    }

    public void removeRadix() throws NoSuchElementException {
        if (radices.isEmpty()) {
            throw new NoSuchElementException();
        }
        maxProduct /= radices.remove(radices.size() - 1);
    }
    ...
    public int[] toRadix(int num) {
        int[] digits = new int[radices.size() + 1];
        int quotient = num;
        for (int i = 0; i < radices.size(); i++) {
            int radix = radices.get(radices.size() - i - 1);
    ...
    public int fromRadix(int[] digits) {
       ...
       */
            if (radices.size() - i - 1 >= 0) {
                product *= radices.get(radices.size() - i - 1);
            }
        }
     ...
     *   and return non-negative digits when appropriate.
     * </p>
     *
     ...
     *
     * @param arrList the int array for this MixedRadix's radices to be assigned to
     */
    public void setRadices(List<Integer> arrList) {
        this.radices = arrList;
    }
    ...
    public void setRadices(String input) throws IllegalArgumentException {
        ...
        for (String radix : radices) {
            if (radix.matches("-?\\d+(\\d+)?")) {
                this.radices.add(Integer.parseInt(radix));
            } else {
                throw new IllegalArgumentException();
            }
        }
     ...
     */
    public ArrayList<Integer> getRadices() {
        return new ArrayList<>(radices);
    }
